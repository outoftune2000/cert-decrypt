...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      for (const batch of fetchedWave) {
        ...
        batch.entries.forEach((entry, entryOffset) => {
          ...
          try {
            ...
            const matchedDomain = Array.from(candidates).find((candidate) => isDomainMatch(candidate, requestedDomain));
            if (!matchedDomain) {
              return;
            }
    ...
    const avgProcessMsPerBatch = searchedBatches > 0 ? Number((processingMsTotal / searchedBatches).toFixed(2)) : 0;
