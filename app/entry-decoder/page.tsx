...
export default function Home() {
  ...
  const [rangeMeta, setRangeMeta] = useState<{ start: number; end: number; provider: CtProvider; count: number } | null>(
    null
  );

  const [seedStart, setSeedStart] = useState("325969000");
  const [seedMeta, setSeedMeta] = useState<{ start: number; end: number; provider: CtProvider; count: number } | null>(
    null
  );
  ...
  const handleFetchAndDecode = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload = await fetchCtEntriesRange(startNumber, endNumber, ctProvider);
      ...
      setRangeMeta({
        ...
        provider: readProviderValue(payload.provider, ctProvider),
        count: payload.count
      });
    } catch (decodeError) {
  ...
  const handleSearchFingerprintMatches = async () => {
    ...
    try {
      ...
      const fetchedBatches = await processInChunks(batchSpecs, SEARCH_FETCH_CONCURRENCY, async (spec) => {
        const payload = await fetchCtEntriesRange(spec.batchStart, spec.batchEnd, seedMeta.provider);
        return {
          ...spec,
          payload
        };
  ...
  return (
    <main className="page">
      <section className="hero">
        ...
        <p>
          Decode CT records, or run a fingerprint-based domain search over rolling windows from supported 2026 logs.
        </p>
        <p className="form-note" style={{ marginTop: "0.8rem" }}>
          Organization-only view: <a href="/organization-only">/organization-only</a> | Range ingest:{" "}
          <a href="/range-ingest">/range-ingest</a> | Domain lookup:{" "}
          <a href="/domain-lookup">/domain-lookup</a> | Descending domain search:{" "}
          <a href="/domain-desc-search">/domain-desc-search</a> | Index compare:{" "}
          <a href="/index-compare">/index-compare</a> | Live tail:{" "}
          <a href="/ct-live-tail">/ct-live-tail</a>
        </p>
      </section>
        ...
        <>
          <form className="decoder-form" onSubmit={handleFetchAndDecode}>
            <div className="range-grid">
              ...
              <div>
                <label htmlFor="decoder-provider">CT Provider</label>
                <select
                  id="decoder-provider"
                  value={ctProvider}
            ...
            <section className="results">
              ...
              <div className="meta-grid">
                ...
                <div>
                  ...
                  <p>{rangeMeta.provider === "digicert" ? "DigiCert" : "Cloudflare"}</p>
                </div>
                <div>
        ...
        <>
          <form className="decoder-form" onSubmit={handleLoadFingerprintSeed}>
            <div className="range-grid">
              ...
              <div>
                <label htmlFor="seed-provider">CT Provider</label>
                <select
                  id="seed-provider"
                  value={ctProvider}
                  onChange={(event) => setCtProvider(event.target.value as CtProvider)}
          ...
          {seedMeta && (
            <section className="results">
              ...
              <div className="meta-grid">
                ...
                <div>
                  <label>Provider</label>
                  <p>{seedMeta.provider === "digicert" ? "DigiCert" : "Cloudflare"}</p>
                </div>
                <div>
